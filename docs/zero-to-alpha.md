...
| 1.3 | Intent Creation page: Intent Card render (editable), Missing Details detection display | opus | high | 1.2 | DONE — `src/app/intents/[id]/page.tsx` loads the owned IntentObject and renders an editable Intent Card (`src/components/intent/IntentCard.tsx`) wired to `applyCompletionAction`, plus a Missing Details panel (`src/components/intent/MissingDetails.tsx`) driven by `MissingFieldService.detect`. Blocker fields are highlighted; status badge + ambiguity notes shown. |
